...
var ConfigError = /** @class */ (function (_super) {
    ...
    function ConfigError(config, propertyName) {
        var _this = this;
        if (!config) {
            throw new MissingArgumentError('You must provide the Phaser config object whose property value is set incorrectly when attempting to create a ConfigError.');
        }
        if (!propertyName) {
            throw new MissingArgumentError('You must provide the name of the property whose value is set incorrectly when attempting to create a ConfigError.');
        }
        var value = config[propertyName];
        if (value === undefined) {
            throw new InvalidPropertyNameError("The Phaser config object does not have a property named " + propertyName + ".");
        }
        _this = _super.call(this, "phaser-react-tools sets the " + propertyName + " property of the Phaser config object internally. As such, the user-provided value of " + value + " will be overridden.") || this;
